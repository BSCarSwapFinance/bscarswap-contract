...
import "./BSCarToken.sol";

// MasterChef is the master of BSCar. He can make BSCar and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once BSCar is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BSARs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBSCarPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBSCarPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BSAR to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BSAR distribution occurs.
        uint256 accBSCarPerShare;   // Accumulated BSAR per share, times 1e12. See below.
    ...
    // The BSAR TOKEN!
    BSCarToken public bscar;
    // Dev address.
    address public devaddr;
    address private devaddrPreventive;
    // Block number when bonus BSAR period ends.
    uint256 public bonusEndBlock;
    // BSAR tokens created per block.
    uint256 public bscarPerBlock;
    // Deposit Fee address
    ...
    // The block number when BSAR mining starts.
    ...
    // Update the given pool's BSAR allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending BSARs on frontend.
    function pendingBSCar(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBSCarPerShare = pool.accBSCarPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 bscarReward = multiplier.mul(bscarPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBSCarPerShare = accBSCarPerShare.add(bscarReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accBSCarPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 bscarReward = multiplier.mul(bscarPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        bscar.mint(devaddr, bscarReward.div(5));
        bscar.mint(devaddrPreventive, bscarReward.div(5));
        bscar.mint(address(this), bscarReward);
        pool.accBSCarPerShare = pool.accBSCarPerShare.add(bscarReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for BSAR allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBSCarPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeBSCarTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accBSCarPerShare).div(1e12);
    ...
    // Safe bscar transfer function, just in case if rounding error causes pool to not have enough BSARs.
    function safeBSCarTransfer(address _to, uint256 _amount) internal {
        uint256 bscarBal = bscar.balanceOf(address(this));
        if (_amount > bscarBal) {
            bscar.transfer(_to, bscarBal);
        } else {
            bscar.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _bscarPerBlock) public onlyOwner {
        bscarPerBlock = _bscarPerBlock;
    }
